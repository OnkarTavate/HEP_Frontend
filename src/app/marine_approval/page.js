...
export default function TrafficPassesPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("marine-card-order");
    if (saved) {
      try {
  ...
  const handleDrop = (e, idx) => {
    ...
    localStorage.setItem("marine-card-order", JSON.stringify(newOrder));
  ...
  const resetCardOrder = () => {
    ...
    localStorage.removeItem("marine-card-order");
