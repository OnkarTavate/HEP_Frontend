              ...
              <tr className="bg-slate-50/50 dark:bg-slate-800/20 border-b border-slate-100 dark:border-slate-700/40">
                {(activeTab === "processed"
                  ...
                  return (
                    <th
                      key={h}
                      className={`px-4 sm:px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider ${h === "Action" || h === "Status" ? "text-center" : ""} ${vis}`}
                    >
                      {h}
                    </th>
                  );
                ...
                displayedRequests.map((req) => {
                  ...
                  return (
                    <tr
                      ...
                      className={rowClass}
                    >
                      <td className="px-4 sm:px-5 py-4 text-sm font-bold text-slate-800 dark:text-stone-200 font-mono hidden sm:table-cell">
                        {req.referenceNumber || "—"}
                      </td>
                      ...
                      {activeTab === "processed" && (
                        <td className="px-4 sm:px-5 py-4 text-sm font-semibold text-slate-600 dark:text-slate-300 hidden lg:table-cell">
                          {req.approvedBy || "—"}
                        </td>
                      )}
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden ring-1 ring-slate-200 dark:ring-white/10">
            {/* Modal header */}
            <div className="flex justify-between items-center gap-2 px-4 sm:px-6 py-4 bg-slate-900 dark:bg-slate-950 text-white">
              <h2 className="text-base sm:text-lg font-bold tracking-wide flex items-center gap-2 sm:gap-2.5 min-w-0">
                <Building2
                  className="text-amber-400 h-5 w-5 shrink-0"
                  strokeWidth={2.5}
                />
                <span className="truncate">
                  ...
                  {selectedRequest.referenceNumber && (
                    <span className="ml-1 text-amber-400 font-mono text-xs sm:text-sm">
                      · {selectedRequest.referenceNumber}
                    </span>
                  )}
                </span>
              ...
              <button
                ...
                className="text-white/60 hover:text-red-400 transition-colors p-1 rounded-lg hover:bg-white/10 shrink-0"
              >
            ...
            {/* Modal footer */}
            {!isViewMode && (
              <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2 sm:gap-3 p-4 border-t border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-900">
                <button
                  onClick={() => handleActionClick("rejected")}
                  className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm shadow hover:shadow-lg active:scale-95 transition-all"
                >
                  <XCircle className="h-4 w-4" /> Reject
                </button>
                <button
                  onClick={handleRevertClick}
                  className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-slate-700 dark:bg-slate-700 hover:bg-slate-800 text-white font-bold text-sm shadow hover:shadow-lg active:scale-95 transition-all"
                >
                  <RefreshCw className="h-4 w-4" /> Revert
                </button>
                <button
                  onClick={() => handleActionClick("approved")}
                  className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm shadow hover:shadow-lg active:scale-95 transition-all"
                >
