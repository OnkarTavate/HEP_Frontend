...
function PaymentModeDonut({ pieChartData }) {
        ...
        <>
              ...
              <PieChart>
                ...
                >
                  {pieChartData.map((entry, idx) => (
                    <Cell
                      key={entry.name}
                      fill={`url(#${GRAD_IDS_TM[idx % GRAD_IDS_TM.length].id})`}
          ...
          <div className="mt-4 space-y-2.5">
            {pieChartData.map((item, idx) => {
              ...
              const grad = GRAD_IDS_TM[idx % GRAD_IDS_TM.length];
              const Icon = SLICE_ICONS_TM[item.name] || CircleDollarSign;
              return (
                <div key={item.name}
