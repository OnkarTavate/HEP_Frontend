...
export default function TrafficBulkPassDetailPage() {
  ...
  if (!batch) {
    return (
      <div className="text-center py-20 text-slate-500">
        Batch not found.{" "}
        <button onClick={() => router.push("/traffic_manager/bulk-pass")} className="text-orange-600 font-semibold hover:underline">Go back to queue</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0a1e4d] via-[#12275f] to-[#1b1856] p-5 sm:p-6 text-white shadow-[0_12px_32px_-10px_rgba(10,30,77,0.65)] ring-1 ring-inset ring-white/15">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push("/traffic_manager/bulk-pass")}
              className="flex items-center justify-center h-10 w-10 rounded-xl bg-white/10 text-white hover:bg-white/20 transition shrink-0">
              <ArrowLeft className="h-5 w-5" />
            </button>
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <h2 className="text-xl sm:text-2xl font-black text-white font-mono tracking-tight">{batch.refNo}</h2>
                <StatusChip status={batch.status} />
              </div>
              <p className="text-[11px] text-blue-200/70 mt-0.5">{batch.companyName} · Created {fmtDate(batch.createdAt)}</p>
            </div>
          </div>
            ...
            {isUnderReview && canApprove && (
              <>
                <button onClick={() => setModal("return")}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-orange-100 bg-orange-500/30 hover:bg-orange-500/50 ring-1 ring-inset ring-orange-300/30 transition">
                  <RotateCcw className="h-4 w-4" /> Return for Revision
                </button>
                <button onClick={() => setModal("reject-batch")}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-red-100 bg-red-500/30 hover:bg-red-500/50 ring-1 ring-inset ring-red-300/30 transition">
            ...
            {isCompleted && (
              <>
                <button onClick={handleViewPass}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-sky-100 bg-sky-500/30 hover:bg-sky-500/50 ring-1 ring-inset ring-sky-300/30 transition">
                  <Eye className="h-4 w-4" /> View Pass
                </button>
                <button onClick={handleDownloadPdf} disabled={!!downloadingId}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-emerald-100 bg-emerald-500/30 hover:bg-emerald-500/50 ring-1 ring-inset ring-emerald-300/30 disabled:opacity-50 disabled:cursor-not-allowed transition">
                  <Download className="h-4 w-4" />
                  {downloadingId ? "Downloading…" : "Download Pass PDF"}
                </button>
                <button onClick={handleResendPass} disabled={resendingPass}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-violet-100 bg-violet-500/30 hover:bg-violet-500/50 ring-1 ring-inset ring-violet-300/30 disabled:opacity-50 disabled:cursor-not-allowed transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl ring-1 ring-slate-200/60 shadow-xl p-6 lg:col-span-2">
        ...
        <div className="bg-white rounded-2xl ring-1 ring-slate-200/60 shadow-xl p-6">
          <div className="flex items-center gap-2 mb-5">
