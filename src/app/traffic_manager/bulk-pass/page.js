        ...
        <tbody>
          {rows.map((batch, idx) => (
            <tr
              ...
              onClick={() => router.push(`/traffic_manager/bulk-pass/${batch.id}`)}
            >
...
export default function TrafficBulkPassPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6">
      {/* ── Header ── */}
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-6 rounded-2xl ring-1 ring-slate-200/50 shadow-md">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-[#0a1e4d] tracking-tight flex items-center gap-2">
            <Users className="h-6 w-6 text-orange-500" strokeWidth={2.5} />
            Bulk Pass Approvals
          </h2>
          <p className="text-sm text-slate-500 mt-1">Review and approve group port-entry pass batches</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 shadow-sm transition active:scale-95"
          >
      ...
      {/* ── Tabs ── */}
      <div className="flex gap-2 border-b border-slate-200 pb-0 overflow-x-auto no-scrollbar">
        <button
        ...
        >
          ...
          {batches.length > 0 && (
            <span className={`ml-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10px] font-bold ${
              activeTab === "queue" ? "bg-white/20 text-white" : "bg-orange-100 text-orange-700"
            }`}>
              {batches.length}
            </span>
          )}
        ...
        {activeTab === "filter" && filterMeta && (
          <div className={`relative flex items-center gap-2 px-6 py-3 text-sm font-bold rounded-t-xl bg-[#0a1e4d] text-white shadow`}>
