...
const DateFilterChips = memo(function DateFilterChips({
  value,
  onChange,
  counts,
}) {
...
const StatCard = memo(function StatCard({
  ...
  return (
    <Card
      className={
        (isPrimary ? cardShellPrimary : cardShell) +
      ...
      <div className="relative px-4 py-3 md:px-5 md:py-3.5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {chip(
            Users,
            "Persons",
            stats.personnelCount + stats.driverCount,
            "text-fuchsia-300",
          )}
          {chip(
            Timer,
            "Expiring",
            stats.expiringSoon,
            stats.expiringSoon > 0 ? "text-red-300" : "text-stone-400",
          )}
        </div>
...
const ExpiringSoonCard = memo(function ExpiringSoonCard({ items, loading }) {
  ...
  return (
    <Card
      className={cardShell + " !hover:translate-y-0 !hover:scale-100 !flex-col"}
    >
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-14 rounded-xl bg-stone-100 dark:bg-white/5 animate-pulse"
              />
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-stone-400">
            ...
            <p className="text-xs mt-1">
              No passes expiring in the next 7 days
            </p>
          </div>
        ) : (
          <>
            <div className="space-y-2">
              {displayed.map((item, i) => {
                ...
                const urgencyClasses = {
                  ...
                  amber: {
                    bg: "bg-amber-50/60 dark:bg-amber-400/5",
                ...
                return (
                  <div
                    key={item.id || i}
                    className={`flex items-center gap-3 rounded-xl ${u.bg} ring-1 ${u.ring} px-3 py-2.5`}
                  >
                    <span
                      className={`flex h-9 w-9 items-center justify-center rounded-lg shrink-0 ${item.type === "vehicle" ? "bg-amber-100 dark:bg-amber-400/15" : "bg-teal-100 dark:bg-teal-400/15"}`}
                    >
                      {item.type === "vehicle" ? (
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-extrabold shrink-0 ${u.badge}`}
                    >
            ...
            {items.length > 5 && (
              ...
              >
                {expanded ? (
                  ...
                  <>
                    <ChevronDown className="h-4 w-4" /> Show {items.length - 5}{" "}
                    More
                  </>
                )}
              </button>
...
// ---------------------------------------------------------------------------
const EntityBreakdownCard = memo(function EntityBreakdownCard({
  stats,
  loading,
}) {
  const rows = [
    {
      ...
      icon: Truck,
      color: "#f59e0b",
      tint: "bg-amber-100 dark:bg-amber-400/15 text-amber-700 dark:text-amber-300",
    },
    {
      key: "personnelCount",
      label: "Personnel",
      icon: Users,
      color: "#14b8a6",
      tint: "bg-teal-100 dark:bg-teal-400/15 text-teal-700 dark:text-teal-300",
    },
    {
      key: "driverCount",
      label: "Drivers",
      icon: CreditCard,
      color: "#a855f7",
      tint: "bg-purple-100 dark:bg-purple-400/15 text-purple-700 dark:text-purple-300",
    },
  ];
      ...
      <CardContent className="space-y-3">
        {loading
          ? [1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-14 rounded-2xl bg-stone-100 dark:bg-white/5 animate-pulse"
              />
            ))
          : rows.map((r) => {
              ...
              return (
                <div
                  key={r.key}
                  className="flex items-center gap-3 rounded-2xl bg-stone-50 dark:bg-white/5 p-3.5"
                >
                  <div
                    className={
...
const ApplicationTrendChart = memo(function ApplicationTrendChart({ data }) {
  ...
  const peak = data.reduce(
    (acc, d) =>
      d.persons + d.vehicles > acc.total
        ? { month: d.month, total: d.persons + d.vehicles }
        : acc,
    { month: "—", total: 0 },
  );
...
const PassTypeChart = memo(function PassTypeChart({ passTypeData }) {
  ...
  const activeSlice =
    activeIndex !== null && passTypeData ? passTypeData[activeIndex] : null;
  ...
  const rows = [
    {
      ...
      icon: CheckCircle,
      color: "#10b981",
      tint: "bg-emerald-100 dark:bg-emerald-400/15 text-emerald-700 dark:text-emerald-300",
    },
    {
      key: "pending",
    ...
    },
    {
      ...
      icon: RefreshCw,
      color: "#fb923c",
      tint: "bg-orange-100 dark:bg-orange-400/15 text-orange-700 dark:text-orange-300",
    },
    {
      key: "rejected",
      label: "Rejected",
      icon: XCircle,
      color: "#ef4444",
      tint: "bg-red-100 dark:bg-red-400/15 text-red-700 dark:text-red-300",
    },
  ];
      ...
      <CardContent className="space-y-3">
        {loading ? (
          [1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-16 rounded-2xl bg-stone-100 dark:bg-white/5 animate-pulse"
            />
          ))
        ) : total === 0 ? (
...
const Field = ({ label, value, mono }) => (
  <div className="min-w-0">
    <p className="text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wider mb-0.5">
      {label}
    </p>
    <p
      className={
        "text-sm font-semibold text-stone-800 dark:text-stone-100 break-words " +
        (mono ? "font-mono" : "")
      }
    >
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      //   String(parsedUser?.role || "").trim() === "Fire Safety Officer" &&
      //   Number(parsedUser?.departmentId) === 7;
  ...
  useEffect(() => {
    const fetchAllPasses = async (headers) => {
      const first = await axios.get(
        `${AGENT_API}/pass-request/my-pass-requests?page=1&limit=100`,
        { headers },
      );
      ...
      if (totalPages > 1) {
        ...
        for (let p = 2; p <= totalPages; p++) {
          reqs.push(
            axios.get(
              `${AGENT_API}/pass-request/my-pass-requests?page=${p}&limit=100`,
              { headers },
            ),
          );
        }
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const profileRes = await axios.get(`${AGENT_API}/agents/profile`, {
          headers: authHeaders,
        });
        if (profileRes.data?.success) {
          ...
          if (data.isBlacklisted) {
            ...
            setBlacklistReason(
              data.blacklistReason || "Suspended due to port violations.",
            );
      ...
      const hepMap = {};
      try {
        const hepRes = await axios.get(
          `${AGENT_API}/pass-request/get-hep-types`,
          { headers: authHeaders },
        );
        (hepRes.data?.data || []).forEach((h) => {
          hepMap[h.id] = String(
            h.hepType || h.name || h.type || h.hep_type || "",
          ).toLowerCase();
        });
      } catch {
        /* fallback */
      ...
      try {
        ...
        console.log(
          "%c[Dashboard] Raw passes from API — full data",
            ...
            if (
              ...
              if (!Number.isNaN(dt.getTime()) && dt >= nowTs && dt <= in7) {
                ...
                expItems.push({
                  ...
                  type: "person",
                  name: p.name || "—",
                  passNo: p.personPassNo,
                  dateTo: p.dateTo,
                  refNo: pr.referenceNo,
                });
              }
            }
          });
        ...
        console.log(
          "%c[Dashboard] Computed apiStats",
          "color:#14b8a6;font-weight:bold",
          computedStats,
        );
        console.log(
          "%c[Dashboard] Status counts",
      ...
      try {
        const penaltyRes = await axios.get(
          `${ADMIN_API}/blacklist/my-blacklist?limit=100`,
          { headers: authHeaders },
        );
        if (penaltyRes.data?.success) {
          ...
          setPenalties(
            rows.filter(
              (r) =>
                r.has_penalty &&
                String(r.penalty_status || "").toUpperCase() === "PENDING",
            ),
          );
        }
      } catch {
  ...
  const filteredPasses = useMemo(
    () => filterPassesByDate(allPasses, dateFilter),
    [allPasses, dateFilter],
  );
  const recentFilteredPasses = useMemo(
    () => filteredPasses.slice(0, 10),
    [filteredPasses],
  );
  ...
  const oldestPendingDays = useMemo(() => {
    if (apiStats.pendingApprovals === 0) {
      console.log(
        "%c[Dashboard] oldestPendingDays → null (no pending approvals)",
        "color:#6b7280",
      );
      return null;
    }
    ...
    console.log(
      "%c[Dashboard] oldestPendingDays",
      "color:#ef4444;font-weight:bold",
      max,
      "days | pendingApprovals:",
      apiStats.pendingApprovals,
    );
    return max;
        ...
        return (
          <StatCard
            ...
            footnote={
              statsLoading
                ? "—"
                : `${apiStats.todayPersons} persons & ${apiStats.todayVehicles} vehicles`
            }
          />
        );
      case "approvalRate":
        const appRate = statsLoading
          ? 0
          : apiStats.totalEntities > 0
            ? Math.round((apiStats.activePasses / apiStats.totalEntities) * 100)
            : 0;
        return (
          <StatCard
            key="approvalRate"
            ...
            footnote={
              statsLoading
                ? "—"
                : `${apiStats.activePasses} approved of ${apiStats.totalEntities}`
            }
      ...
      case "pendingApprovals":
        return (
          <StatCard
            ...
            footnote={
              apiStats.pendingApprovals > 0
                ? oldestPendingDays !== null
                  ? `Oldest: ${fmtAge(oldestPendingDays)} ago`
                  : "Awaiting review"
                : "All up to date"
            }
      ...
      <div className="relative space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
          <div>
            ...
            <p className="text-base md:text-lg text-stone-600 dark:text-stone-400 max-w-2xl">
              Here&rsquo;s your live port-pass overview &amp; compliance
              summary.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Button
              onClick={() =>
                isBlacklisted
                  ? (setShowBlacklistPopup(true),
                    toast.error("Blocked: Your company is blacklisted."))
        ...
        <div className="flex items-center justify-between gap-3 text-xs bg-amber-500/5 rounded-2xl px-4 py-2 border border-amber-500/10">
          <p className="text-stone-600 dark:text-amber-400/80 font-medium flex items-center gap-1.5">
            ...
            Drag and drop card handles (grip icon) to arrange stats. Persistence
            saved!
          </p>
          <button
            ...
            return (
              <div
                ...
                className={
                  ...
                  (isOver
                    ? " border-2 border-dashed border-amber-500 rounded-3xl p-1 bg-amber-500/5 shadow-inner scale-[1.02]"
                    : "")
                }
              >
                {renderCard(cardKey)}
              ...
              <Card className={cardShellAlertRed + " lg:col-span-2"}>
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <CardDescription className="text-red-600/80 dark:text-red-300/80 text-sm font-medium">
                ...
                <CardContent className="space-y-3">
                  <div className="bg-red-50 dark:bg-red-500/10 p-3 rounded-2xl ring-1 ring-red-200/60 dark:ring-red-500/20">
                    <p className="text-red-700 dark:text-red-300 text-sm leading-relaxed line-clamp-2">
                  ...
                  <div className="flex items-center justify-end text-sm">
                    <Button
                      size="sm"
                      onClick={() =>
                        router.push("/dashboard/blacklist_penalties")
                      }
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge className="bg-amber-600 text-white border-0 text-xs font-bold px-2 py-0.5 flex-shrink-0 rounded-full">
                          {penalties.length} VIOLATION
                          {penalties.length !== 1 ? "S" : ""}
                        </Badge>
                      </div>
                      <CardDescription className="text-amber-600/80 dark:text-amber-300/80 text-sm font-medium">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card
            className={cardShell + " !hover:translate-y-0 !hover:scale-100"}
          >
            <CardHeader className="pb-4">
              ...
              {statsLoading ? (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="h-20 rounded-2xl bg-stone-100 dark:bg-white/5 animate-pulse"
                    />
                  ))}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-stone-400">
                  ...
                  <p className="text-sm font-semibold">
                    {dateFilter === "today"
                      ? "No passes applied today"
                      : dateFilter === "week"
                        ? "No passes this week"
                        : "No pass applications yet"}
                  </p>
                  <p className="text-xs mt-1">
                    {dateFilter !== "all" ? (
                      <button
                        onClick={() => setDateFilter("all")}
                        className="text-amber-600 dark:text-amber-400 underline font-semibold"
                      >
                        View all passes
                      </button>
                    ) : (
                      'Click "Apply New Pass" to get started'
                    )}
                  </p>
                </div>
              ) : (
                recentFilteredPasses.map((pass, idx) => {
                  const persons = Array.isArray(pass.persons)
                    ? pass.persons
                    : [];
                  const vehicles = Array.isArray(pass.vehicles)
                    ? pass.vehicles
                    : [];
                  ...
                  const latestTo = allDates.length
                    ? new Date(Math.max(...allDates))
                    : null;
                  const daysLeft = latestTo
                    ? daysUntil(latestTo.toISOString())
                    : null;
                  const hasExpiring =
                  ...
                  return (
                    <div
                      ...
                      className={
                        "flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-2xl hover:bg-amber-50 dark:hover:bg-amber-400/10 hover:ring-1 hover:ring-amber-300/50 transition-all gap-3 cursor-pointer group " +
                        (hasExpiring
                          ? "bg-red-50/50 dark:bg-red-400/5 ring-1 ring-red-200/60 dark:ring-red-400/10"
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-stone-900 dark:text-stone-100">
                            {pass.referenceNo || `PASS-${idx + 1}`}
                          </p>
                          <p className="text-sm text-stone-500 dark:text-stone-400 font-medium">
                            {persons.length} person
                            {persons.length !== 1 ? "s" : ""} &bull;{" "}
                            {vehicles.length} vehicle
                            {vehicles.length !== 1 ? "s" : ""}
                          </p>
                          <p className="text-xs text-stone-400 dark:text-stone-500 font-semibold">
                            Submitted{" "}
                            {formatDate(pass.createdAt || pass.submittedAt)}
                          </p>
                        </div>
                      </div>
                      <div className="text-left sm:text-right w-full sm:w-auto">
                        <Badge
                          className={`${meta.color} border text-xs font-bold px-3 py-1 rounded-full`}
                        >
                        ...
                        {hasExpiring ? (
                          ...
                          <p className="text-sm text-stone-500 dark:text-stone-400 font-medium mt-1.5">
                            Valid until {formatDate(latestTo)}
                          </p>
                        ) : null}
                      </div>
                    </div>
        ...
        {penalties.length > 0 && (
          ...
          >
            <CardHeader className="pb-4">
              ...
              <CardDescription className="text-base text-stone-500 dark:text-stone-400 mt-1">
                All outstanding penalties associated with your company
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-stone-500 dark:text-stone-400 font-medium mt-0.5">
                          {pen.reason || pen.reason_code || "Violation"} ·{" "}
                          {pen.penalty_status || "PENDING"}
                        </p>
                      </div>
                    </div>
                    <p className="font-bold text-lg tabular-nums text-red-600 dark:text-red-400">
                      ₹
                      {parseFloat(pen.penalty_amount || 0).toLocaleString(
                        "en-IN",
                      )}
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 bg-[#0f172a]/75 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1e293b] rounded-3xl max-w-lg w-full p-6 md:p-8 relative shadow-2xl border border-red-500/30 overflow-hidden flex flex-col items-center text-center">
            ...
            <p className="text-sm md:text-base text-stone-600 dark:text-stone-300 leading-relaxed max-w-md mb-6">
              ...
              ) has been blacklisted from Chennai Port.
            </p>
            <div className="w-full bg-stone-50 dark:bg-stone-900/50 rounded-2xl p-4 border border-stone-200/60 dark:border-white/5 text-left mb-6">
              ...
              <p className="text-sm font-semibold text-stone-700 dark:text-stone-200 leading-relaxed">
                {blacklistReason ||
                  "Reason not specified. Please contact administration."}
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Button
                ...
                className="flex-1 rounded-2xl h-12 bg-stone-900 hover:bg-stone-800 dark:bg-white dark:text-stone-900 dark:hover:bg-stone-100 text-white font-bold"
              >
                View Penalties
              </Button>
              <Button
        ...
        <div className="fixed inset-0 bg-[#0f172a]/75 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1e293b] rounded-3xl max-w-lg w-full p-6 md:p-8 relative shadow-2xl border border-amber-500/30 overflow-hidden flex flex-col items-center text-center">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/20 text-xs font-bold uppercase tracking-wider mb-4">
              <AlertCircle className="h-3.5 w-3.5" strokeWidth={2.5} /> Vehicle
              Suspensions
            </div>
            <p className="text-sm md:text-base text-stone-600 dark:text-stone-300 leading-relaxed max-w-md mb-6">
              The following vehicle(s) registered under your company have been
              blacklisted and will be excluded from new pass requests.
            </p>
            <div className="w-full max-h-48 overflow-y-auto space-y-3 mb-6 pr-1 [scrollbar-width:thin]">
              {blacklistedVehicles.map((veh, i) => (
                <div
                  key={i}
                  className="bg-stone-50 dark:bg-stone-900/50 rounded-2xl p-4 border border-stone-200/60 dark:border-white/5 text-left"
                >
                  <div className="flex items-center justify-between border-b border-stone-200/60 dark:border-white/5 pb-2 mb-2">
                    <span className="text-sm font-extrabold text-stone-800 dark:text-stone-100 font-mono">
                      {veh.identifier}
                    </span>
                    <span className="text-[10px] font-bold text-red-600 bg-red-50 dark:bg-red-500/10 px-2 py-0.5 rounded border border-red-200/50 uppercase tracking-wide">
                      {veh.status}
                    </span>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Button
                onClick={() => setShowVehicleWarningPopup(false)}
                className="flex-1 rounded-2xl h-12 bg-amber-600 hover:bg-amber-700 text-white font-bold"
              >
                Close Alert
              </Button>
              <Button
...
// ---------------------------------------------------------------------------
const PassDetailModal = memo(function PassDetailModal({
  pass,
  hepTypes,
  onClose,
}) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/70 backdrop-blur-md p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#1f232d] w-full max-w-3xl rounded-3xl shadow-2xl ring-1 ring-stone-200/60 dark:ring-white/10 overflow-hidden animate-in zoom-in-95 duration-200 max-h-[92vh] flex flex-col">
        <div className="flex justify-between items-center px-5 sm:px-6 py-4 bg-gradient-to-r from-[#1f1f1f] via-[#2a2520] to-[#3a2f1f] text-white shrink-0">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`hidden sm:inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold border ${reqMeta.color}`}
            >
        ...
        <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1 [scrollbar-width:thin]">
          ...
          <div className="rounded-2xl bg-stone-50 dark:bg-white/5 ring-1 ring-stone-200/70 dark:ring-white/10 p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <Field
                label="Submitted"
                value={formatDate(pass.submittedAt || pass.createdAt)}
              />
              <Field
                label="Gross Total"
                value={
                  pass.grossTotal != null
                    ? formatCurrency(parseFloat(pass.grossTotal))
                    : "—"
                }
              />
              <Field
                ...
                value={
                  pass.gstAmount != null
                    ? formatCurrency(parseFloat(pass.gstAmount))
                    : "—"
                }
              />
              <Field
                ...
                value={
                  pass.netAmount != null
                    ? formatCurrency(parseFloat(pass.netAmount))
                    : "—"
                }
              />
            </div>
              ...
              <div className="mt-3 pt-3 border-t border-stone-200/70 dark:border-white/10">
                <PassDetailModalDocs
                  docs={[
                    {
                      label:
                        pass.requisitionLetterFileName || "Requisition Letter",
                      path: pass.requisitionLetterFilePath,
                    },
                  ]}
                />
              </div>
            ...
            <div className="space-y-3">
              ...
              {persons.map((p, i) => {
                ...
                return (
                  <div
                    key={p.id || i}
                    className="rounded-2xl bg-white dark:bg-white/[0.03] ring-1 ring-stone-200/70 dark:ring-white/10 p-4"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-stone-500 dark:text-stone-400 font-mono">
                            {p.personPassNo || "Pass no. pending"}
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border ${meta.color}`}
                        >
                          {meta.label.toUpperCase()}
                        </span>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <Field
                        ...
                        value={
                          hep
                            ? hep.charAt(0).toUpperCase() + hep.slice(1)
                            : (p.hepTypeId ?? "—")
                        }
                      />
                    ...
                    {p.status === "reverted" && p.rejectedReason && (
                      <div className="mt-3 rounded-xl bg-orange-50 dark:bg-orange-400/10 ring-1 ring-orange-200/70 dark:ring-orange-400/20 p-2.5">
                        <p className="text-[10px] font-bold text-orange-600 dark:text-orange-300 uppercase tracking-wider mb-0.5">
                          Revert / Reject Reason
                        </p>
                      ...
                      <PassDetailModalDocs
                        docs={[
                          ...
                          { label: "ID Proof", path: p.idProofFilePath },
                          {
                            ...
                            path: p.driverLicensePath,
                          },
                          {
                            label: "Police Verification",
                            path: p.policeVerificationPath,
                          },
                          {
                            label: "Employment Proof",
                            path: p.employmentProofPath,
                          },
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-stone-900 dark:text-stone-100 truncate font-mono">
                            {v.registrationNo || "—"}
                          </p>
                          <p className="text-xs text-stone-500 dark:text-stone-400 font-mono">
                            {v.vehiclePassNo || "Pass no. pending"}
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      ...
                      <Field
                        label="Validity"
                        value={dateRange(v.dateFrom, v.dateTo)}
                      />
                      <Field
                        label="Amount"
                        value={
                    ...
                    <div className="mt-3 pt-3 border-t border-stone-200/70 dark:border-white/10">
                      ...
                      <PassDetailModalDocs
                        docs={[
                          {
                            label: "RC / Scanned Copy",
                            path: v.scannedCopyFilePath,
                          },
          ...
          {persons.length === 0 && vehicles.length === 0 && (
            <div className="py-10 text-center text-stone-400">
              ...
              <p className="text-sm font-semibold">
                No persons or vehicles on this request
              </p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 sm:px-6 py-4 border-t border-stone-200/70 dark:border-white/10 bg-stone-50 dark:bg-white/[0.02] shrink-0 flex justify-end">
          <Button
            onClick={onClose}
            className="rounded-xl bg-stone-900 hover:bg-stone-800 dark:bg-white dark:text-stone-900 dark:hover:bg-stone-100 text-white font-bold px-6"
          >
            Close
          </Button>
        </div>
