...
import {
  ...
  CheckSquare, FileStack, FileText, Car, CalendarDays, Eye,
} from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        ...
        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Return Reason <span className="text-red-500">*</span></label>
...
function ActionBtn({ batch, downloadingId, onReturn, onEdit, onDownloadPdf }) {
  ...
  if (status === "RETURNED_TO_APPLICANT")
    return <button onClick={() => onEdit(batch)} className={`${base} border-purple-200 text-purple-700 bg-purple-50 hover:bg-purple-100`}><Edit3 className="h-3.5 w-3.5" />Edit &amp; Resubmit</button>;
  if (status === "UNDER_REVIEW")
    return <button onClick={() => onEdit(batch)} className={`${base} border-amber-300 text-amber-800 bg-amber-50 hover:bg-amber-100`}><Eye className="h-3.5 w-3.5" />View</button>;
  if (status === "REJECTED")
...
export default function BulkPassListPage() {
  ...
  // Colored stat cards — each is also the filter tab
  const CARDS = [
    { key: "ALL",                   label: "Total Requests",     value: summary.totalBatches ?? 0,
      active: "bg-slate-900 text-white shadow-lg shadow-slate-900/20",
      inactive: "bg-white border border-slate-200 hover:border-slate-300 hover:shadow-sm",
    ...
    { key: "UNDER_REVIEW",          label: "Pending Approval",   value: summary.underReview ?? 0,
      active: "bg-amber-500 text-white shadow-lg shadow-amber-500/30",
      inactive: "bg-amber-50 border border-amber-200 hover:border-amber-300 hover:shadow-sm",
    ...
    { key: "RETURNED_TO_APPLICANT", label: "Returned",           value: summary.returned ?? 0,
      active: "bg-purple-600 text-white shadow-lg shadow-purple-600/30",
      inactive: "bg-purple-50 border border-purple-200 hover:border-purple-300 hover:shadow-sm",
    ...
    { key: "REJECTED",              label: "Rejected",           value: summary.rejected ?? 0,
      active: "bg-red-500 text-white shadow-lg shadow-red-500/30",
      inactive: "bg-red-50 border border-red-200 hover:border-red-300 hover:shadow-sm",
    ...
    { key: "COMPLETED",             label: "Approved",           value: summary.completed ?? 0,
      active: "bg-emerald-500 text-white shadow-lg shadow-emerald-500/30",
      inactive: "bg-emerald-50 border border-emerald-200 hover:border-emerald-300 hover:shadow-sm",
    ...
    { key: "DRAFT",                 label: "Sent to Applicant",  value: summary.draft ?? 0,
      active: "bg-sky-500 text-white shadow-lg shadow-sky-500/30",
      inactive: "bg-sky-50 border border-sky-200 hover:border-sky-300 hover:shadow-sm",
        ...
        {CARDS.map((card) => {
          ...
          return (
            <button key={card.key}
              ...
              className={`relative flex flex-col gap-2 px-4 py-4 rounded-2xl text-left transition-all duration-150 active:scale-[0.97] ${isActive ? card.active : card.inactive}`}>
                  ...
                  {paginated.map((batch) => (
                    <tr key={batch.id}
                      ...
                      <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                        <ActionBtn batch={batch} downloadingId={downloadingId}
